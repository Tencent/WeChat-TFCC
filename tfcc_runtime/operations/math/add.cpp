...
template <class T>
static const char* _wrapper_add_v1(
...
template <class T>
static const char* _wrapper_add_v2(
...
template <class T>
static const char* _wrapper_add_v3(
...
template <class T>
static const char* _wrapper_add_v4(const T* a, const T* b, T* result) noexcept {
  *result = *a + *b;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Add<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Add operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Add<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Add::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Add<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Add>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_add_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Add<dtype>()));
