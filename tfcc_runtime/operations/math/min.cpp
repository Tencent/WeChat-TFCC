...
template <class T>
static const char* _wrapper_min_v2(
...
template <class T>
static const char* _wrapper_min_v3(
...
template <class T>
static const char* _wrapper_min_v4(const T* a, const T* b, T* result) noexcept {
  *result = std::min(*a, *b);
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Min<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Min operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Min<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Min::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Min<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Min>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_min_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Min<dtype>()));
