...
template <class T>
static const char* _wrapper_reduce_min(
  ...
  try {
    ...
    if (needTranspose) {
      ...
      *result = tfcc::math::reduce_min(*a, perm.size() - axes->size());
    }
...
std::string ReduceMin<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::ReduceMin operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ReduceMin<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::ReduceMin::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ReduceMin<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::ReduceMin>()) {
    return nullptr;
  }
  ...
  std::unique_ptr<ReduceMinResource> resource(new ReduceMinResource);
  tfcc::runtime::operations::math::ReduceMin operation;
  ...
  callFunction(jit, _wrapper_reduce_min<T>, inputSymbol, &resource->axes, outputSymbol);

  return std::unique_ptr<OperationResource>(std::move(resource));
}

std::vector<std::unique_ptr<Operation>> get_reduce_min_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ReduceMin<dtype>()));
