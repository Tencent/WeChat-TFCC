...
template <class T>
static const char* _wrapper_div_v1(
...
template <class T>
static const char* _wrapper_div_v2(
...
template <class T>
static const char* _wrapper_div_v3(
...
template <class T>
static const char* _wrapper_div_v4(const T* a, const T* b, T* result) noexcept {
  *result = *a / *b;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Div<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Div operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Div<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Div::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Div<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Div>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_div_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Div<dtype>()));
