...
template <class T>
static const char* _wrapper_reduce_mean(
  ...
  try {
    ...
    if (needTranspose) {
      ...
      *result = tfcc::math::reduce_mean(*a, perm.size() - axes->size());
    }
...
std::string ReduceMean<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::ReduceMean operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ReduceMean<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::ReduceMean::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ReduceMean<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::ReduceMean>()) {
    return nullptr;
  }
  ...
  std::unique_ptr<ReduceMeanResource> resource(new ReduceMeanResource);
  tfcc::runtime::operations::math::ReduceMean operation;
  ...
  callFunction(jit, _wrapper_reduce_mean<T>, inputSymbol, &resource->axes, outputSymbol);

  return std::unique_ptr<OperationResource>(std::move(resource));
}

std::vector<std::unique_ptr<Operation>> get_reduce_mean_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ReduceMean<dtype>()));
