...
template <class T>
static const char* _wrapper_sigmoid(const tfcc::Tensor<T>* a, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::sigmoid(*a);
...
std::string Sigmoid<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Sigmoid operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Sigmoid<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Sigmoid::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Sigmoid<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Sigmoid>()) {
    return nullptr;
  }
  ...
  callFunction(jit, _wrapper_sigmoid<T>, inputSymbol, outputSymbol);

  return resource;
}

std::vector<std::unique_ptr<Operation>> get_sigmoid_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new Sigmoid<dtype>()));
