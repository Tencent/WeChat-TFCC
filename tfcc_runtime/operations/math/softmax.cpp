...
template <class T>
static const char* _wrapper_softmax(
    const tfcc::Tensor<T>* a, size_t axis, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::softmax(*a, axis);
...
std::string Softmax<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Softmax operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Softmax<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Softmax::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Softmax<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Softmax>()) {
    return nullptr;
  }
  ...
  callFunction(jit, _wrapper_softmax<T>, inputSymbol, operation.axis(), outputSymbol);

  return resource;
}

std::vector<std::unique_ptr<Operation>> get_softmax_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new Softmax<dtype>()));
