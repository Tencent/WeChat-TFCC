...
template <class T>
static const char* _wrapper_pow_v1(
    const tfcc::Tensor<T>* a, const tfcc::Tensor<T>* exponent, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::pow(*a, *exponent);
...
template <class T>
static const char* _wrapper_pow_v2(
    const tfcc::Tensor<T>* a, const T* exponent, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::pow(*a, *exponent);
  } catch (std::exception& e) {
    thread_local std::string reason = e.what();
...
template <class T>
static const char* _wrapper_pow_v3(
    const T* a, const tfcc::Tensor<T>* exponent, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::pow(*a, *exponent);
  } catch (std::exception& e) {
    thread_local std::string reason = e.what();
...
template <class T>
static const char* _wrapper_pow_v4(const T* a, const T* exponent, T* result) noexcept {
  *result = static_cast<T>(::pow(*a, *exponent));
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Pow<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Pow operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Pow<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Pow::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Pow<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Pow>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_pow_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Pow<dtype>()));
