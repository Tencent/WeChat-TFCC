...
template <class T>
static const char* _wrapper_reduce_max(
  ...
  try {
    ...
    if (needTranspose) {
      ...
      *result = tfcc::math::reduce_max(*a, perm.size() - axes->size());
    }
...
std::string ReduceMax<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::ReduceMax operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ReduceMax<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::ReduceMax::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ReduceMax<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::ReduceMax>()) {
    return nullptr;
  }
  ...
  std::unique_ptr<ReduceMaxResource> resource(new ReduceMaxResource);
  tfcc::runtime::operations::math::ReduceMax operation;
  ...
  callFunction(jit, _wrapper_reduce_max<T>, inputSymbol, &resource->axes, outputSymbol);

  return std::unique_ptr<OperationResource>(std::move(resource));
}

std::vector<std::unique_ptr<Operation>> get_reduce_max_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ReduceMax<dtype>()));
