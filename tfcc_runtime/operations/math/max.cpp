...
template <class T>
static const char* _wrapper_max_v2(
...
template <class T>
static const char* _wrapper_max_v3(
...
template <class T>
static const char* _wrapper_max_v4(const T* a, const T* b, T* result) noexcept {
  *result = std::max(*a, *b);
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Max<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Max operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Max<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Max::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Max<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Max>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_max_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Max<dtype>()));
