...
template <class T>
static const char* _wrapper_clip(
    const tfcc::Tensor<T>* a, const T* minVal, const T* maxVal,
    tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::clip(*a, *minVal, *maxVal);
...
std::string Clip<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Clip operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Clip<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Clip::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Clip<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 3 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Clip>()) {
    return nullptr;
  }
  ...
  if (symbolManager.getSymbolInfo(node.inputs(1)).dtype != DTYPE ||
  ...
  const T* minValSymbol = symbolManager.getValue(node.inputs(1), T());
  const T* maxValSymbol = symbolManager.getValue(node.inputs(2), T());
  tfcc::Variable<T>* outputSymbol = symbolManager.getVariable(node.outputs(0), T());

  callFunction(jit, _wrapper_clip<T>, inputSymbol, minValSymbol, maxValSymbol, outputSymbol);

  return resource;
}

std::vector<std::unique_ptr<Operation>> get_clip_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Clip<dtype>()));
