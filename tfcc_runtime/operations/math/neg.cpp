...
template <class T>
static const char* _wrapper_neg_v1(const tfcc::Tensor<T>* a, tfcc::Variable<T>* result) noexcept {
  try {
    *result = static_cast<T>(0) - *a;
...
template <class T>
static const char* _wrapper_neg_v2(const T* a, T* result) noexcept {
  try {
    *result = static_cast<T>(0) - *a;
  } catch (std::exception& e) {
    thread_local std::string reason = e.what();
...
std::string Neg<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Neg operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Neg<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Neg::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Neg<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Neg>()) {
    return nullptr;
  }
  ...
  if (is_tensor(inputInfo.stype) && is_tensor(outputInfo.stype)) {
    ...
    callFunction(jit, _wrapper_neg_v1<T>, inputSymbol, outputSymbol);
    ...
    callFunction(jit, _wrapper_neg_v2<T>, inputSymbol, outputSymbol);
  } else {
    // stype error
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_neg_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Neg<dtype>()));
