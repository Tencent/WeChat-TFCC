...
template <class T>
static const char* _wrapper_tanh(const tfcc::Tensor<T>* a, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::tanh(*a);
...
std::string Tanh<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Tanh operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Tanh<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Tanh::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Tanh<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Tanh>()) {
    return nullptr;
  }
  ...
  callFunction(jit, _wrapper_tanh<T>, inputSymbol, outputSymbol);

  return resource;
}

std::vector<std::unique_ptr<Operation>> get_tanh_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Tanh<dtype>()));
