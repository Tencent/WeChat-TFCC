...
template <class T>
static const char* _wrapper_rsqrt_v1(const tfcc::Tensor<T>* a, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::math::rsqrt(*a);
...
template <class T>
static const char* _wrapper_rsqrt_v2(const T* a, T* result) noexcept {
  *result = static_cast<T>(1) / static_cast<T>(std::sqrt(*a));
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Rsqrt<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::Rsqrt operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Rsqrt<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::Rsqrt::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Rsqrt<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::Rsqrt>()) {
    return nullptr;
  }
  ...
  if (is_tensor(inputInfo.stype) && is_tensor(outputInfo.stype)) {
    ...
    callFunction(jit, _wrapper_rsqrt_v1<T>, inputSymbol, outputSymbol);
    ...
    callFunction(jit, _wrapper_rsqrt_v2<T>, inputSymbol, outputSymbol);
  } else {
    // error
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_rsqrt_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Rsqrt<dtype>()));
