...
template <class T>
static const char* _wrapper_reduce_sum(
  ...
  try {
    ...
    if (needTranspose) {
      ...
      *result = tfcc::math::reduce_sum(*a, perm.size() - axes->size());
    }
...
std::string ReduceSum<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::math::ReduceSum operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ReduceSum<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::math::ReduceSum::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ReduceSum<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::math::ReduceSum>()) {
    return nullptr;
  }
  ...
  std::unique_ptr<ReduceSumResource> resource(new ReduceSumResource);
  tfcc::runtime::operations::math::ReduceSum operation;
  ...
  callFunction(jit, _wrapper_reduce_sum<T>, inputSymbol, &resource->axes, outputSymbol);

  return std::unique_ptr<OperationResource>(std::move(resource));
}

std::vector<std::unique_ptr<Operation>> get_reduce_sum_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ReduceSum<dtype>()));
