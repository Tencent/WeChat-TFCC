...
std::string AveragePool1D<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::nn::AveragePool1D operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> AveragePool1D<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::nn::AveragePool1D::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> AveragePool1D<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::nn::AveragePool1D>()) {
    return nullptr;
  }
  ...
  callFunction(
      jit, _wrapper_average_pool1d<T>, inputSymbol, operation.kernel(), operation.padding(),
      operation.stride(), outputSymbol);
  return resource;
}

std::vector<std::unique_ptr<Operation>> get_average_pool1d_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new AveragePool1D<dtype>()));
