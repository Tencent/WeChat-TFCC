...
template <class T>
static const char* _wrapper_to_tensor_v1(
...
template <class T>
static const char* _wrapper_to_tensor_v2(
    const std::vector<T>* a, tfcc::Variable<T>* result) noexcept {
  try {
    *result = tfcc::data::set(*a, {static_cast<unsigned>(a->size())});
  } catch (std::exception& e) {
    thread_local std::string reason = e.what();
    return reason.c_str();
  }
...
template <class T>
static const char* _wrapper_to_tensor_v3(const T* a, tfcc::Variable<T>* result) noexcept {
  try {
    std::vector<T> val = {*a};
    *result = tfcc::data::set(val, {1u});
  } catch (std::exception& e) {
    thread_local std::string reason = e.what();
    return reason.c_str();
  }
...
std::string ToTensor<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::base::ToTensor operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ToTensor<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::base::ToTensor::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ToTensor<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::base::ToTensor>()) {
    return nullptr;
  }
  ...
  if (is_tensor(inputInfo.stype)) {
    ...
    callFunction(jit, _wrapper_to_tensor_v1<T>, inputSymbol, outputSymbol);
  } else if (is_vector(inputInfo.stype)) {
    const std::vector<T>* inputSymbol = symbolManager.getVector(node.inputs(0), T());
    tfcc::Variable<T>* outputSymbol = symbolManager.getVariable(node.outputs(0), T());
    callFunction(jit, _wrapper_to_tensor_v2<T>, inputSymbol, outputSymbol);
  } else if (is_value(inputInfo.stype)) {
    const T* inputSymbol = symbolManager.getValue(node.inputs(0), T());
    tfcc::Variable<T>* outputSymbol = symbolManager.getVariable(node.outputs(0), T());
    callFunction(jit, _wrapper_to_tensor_v3<T>, inputSymbol, outputSymbol);
  } else {
    // error
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_to_tensor_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ToTensor<dtype>()));
