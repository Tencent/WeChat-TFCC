...
template <class T>
static const char* _wrapper_to_value_v1(const tfcc::Tensor<T>* a, T* result) noexcept {
  if (a->size() != 1) {
    return "ToValue: invalid size of input";
  }
  try {
    *result = tfcc::data::get(*a)[0];
...
template <class T>
static const char* _wrapper_to_value_v2(const std::vector<T>* a, T* result) noexcept {
  if (a->size() != 1) {
    return "ToValue: invalid size of input";
  }
  *result = (*a)[0];
...
template <class T>
static const char* _wrapper_to_value_v3(const T* a, T* result) noexcept {
  *result = *a;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string ToValue<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::base::ToValue operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> ToValue<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::base::ToValue::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> ToValue<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 1 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::base::ToValue>()) {
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_to_value_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new ToValue<dtype>()));
