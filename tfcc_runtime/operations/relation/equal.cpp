...
template <class T>
static const char* _wrapper_equal_v2(
...
template <class T>
static const char* _wrapper_equal_v3(
...
template <class T>
static const char* _wrapper_equal_v4(const T* a, const T* b, uint8_t* result) noexcept {
  *result = *a == *b ? 1 : 0;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string Equal<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::relation::Equal operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> Equal<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::relation::Equal::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> Equal<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::relation::Equal>()) {
    return nullptr;
  }
  ...
  if (is_tensor(input0Info.stype) && is_tensor(input1Info.stype)) {
    ...
    callFunction(jit, _wrapper_equal_v4<T>, input0Symbol, input1Symbol, outputSymbol);
  } else {
    // error
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_equal_operations() {
#define DEFINE_FUNC(dtype) operations.emplace_back(std::unique_ptr<Operation>(new Equal<dtype>()));
