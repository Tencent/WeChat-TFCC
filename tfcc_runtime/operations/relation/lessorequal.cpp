...
template <class T>
static const char* _wrapper_less_or_equal_v2(
...
template <class T>
static const char* _wrapper_less_or_equal_v3(
...
template <class T>
static const char* _wrapper_less_or_equal_v4(const T* a, const T* b, uint8_t* result) noexcept {
  *result = *a <= *b ? 1 : 0;
  return nullptr;
}

template <tfcc::runtime::common::DataType DTYPE>
std::string LessOrEqual<DTYPE>::getOperationTypeUrl() const {
  tfcc::runtime::operations::relation::LessOrEqual operation;
  return get_protobuf_type_url(operation);
}

template <tfcc::runtime::common::DataType DTYPE>
std::set<unsigned> LessOrEqual<DTYPE>::getOperationVersions() const {
  std::set<unsigned> versions = {tfcc::runtime::operations::relation::LessOrEqual::VERSION_1};
  return versions;
}

template <tfcc::runtime::common::DataType DTYPE>
std::unique_ptr<OperationResource> LessOrEqual<DTYPE>::process(
    const tfcc::runtime::model::Node& node, Graph& graph) const {
  if (node.inputs_size() != 2 || node.outputs_size() != 1) {
    return nullptr;
  }
  if (!node.operation().Is<tfcc::runtime::operations::relation::LessOrEqual>()) {
    return nullptr;
  }
  ...
  if (is_tensor(input0Info.stype) && is_tensor(input1Info.stype)) {
    ...
    callFunction(jit, _wrapper_less_or_equal_v1<T>, input0Symbol, input1Symbol, outputSymbol);
  } else if (is_tensor(input0Info.stype) && is_value(input1Info.stype)) {
    // variable - value
    const tfcc::Tensor<T>* input0Symbol = symbolManager.getTensor(node.inputs(0), T());
    const T* input1Symbol = symbolManager.getValue(node.inputs(1), T());
    tfcc::Variable<uint8_t>* outputSymbol = symbolManager.getVariable(node.outputs(0), uint8_t());
    callFunction(jit, _wrapper_less_or_equal_v2<T>, input0Symbol, input1Symbol, outputSymbol);
  } else if (is_value(input0Info.stype) && is_tensor(input1Info.stype)) {
    // variable - value
    const T* input0Symbol = symbolManager.getValue(node.inputs(0), T());
    const tfcc::Tensor<T>* input1Symbol = symbolManager.getTensor(node.inputs(1), T());
    tfcc::Variable<uint8_t>* outputSymbol = symbolManager.getVariable(node.outputs(0), uint8_t());
    callFunction(jit, _wrapper_less_or_equal_v3<T>, input0Symbol, input1Symbol, outputSymbol);
    ...
    callFunction(jit, _wrapper_less_or_equal_v4<T>, input0Symbol, input1Symbol, outputSymbol);
  } else {
    // error
    return nullptr;
  }
...
std::vector<std::unique_ptr<Operation>> get_less_or_equal_operations() {
#define DEFINE_FUNC(dtype) \
  operations.emplace_back(std::unique_ptr<Operation>(new LessOrEqual<dtype>()));
