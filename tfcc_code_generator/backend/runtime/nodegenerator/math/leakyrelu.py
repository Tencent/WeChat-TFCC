...
class LeakyRelu(NodeGenerator):
    @classmethod
    def accept(cls, node: ir.node.Node):
        return isinstance(node, ir.node.math.LeakyRelu)

    @property
    def operation(self):
        operation = math_pb2.LeakyRelu()
        if self.outputs[0].is_integer():
            operation.alpha.int64_value = self.node.alpha
        elif self.outputs[0].is_floating_point():
            operation.alpha.double_value = self.node.alpha
        else:
