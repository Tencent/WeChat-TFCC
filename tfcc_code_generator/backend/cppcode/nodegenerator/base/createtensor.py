...
class CreateTensor(NodeGenerator):
    ...
    @property
    def code(self):
        value = get_cpp_constant_value(self.node.value.tolist()[0], self.node.dtype)
        return "auto {outputs[0]} = tfcc::helper::base::create_tensor({value}, {inputs[0]});\n".format(
            value=value, **self.fmt_dict
        )
