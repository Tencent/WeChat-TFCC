...
class ToValue(NodeGenerator):
    @classmethod
    def accept(cls, node: ir.node.Node):
        return isinstance(node, ir.node.base.ToValue)

    @property
    def code(self):
        if self.inputs[0].is_tensor():
            return "auto {outputs[0]} = tfcc::data::get({inputs[0]})[0];\n".format(
                **self.fmt_dict
            )
        elif self.inputs[0].is_value():
            return "{outputs[0].dtype} {outputs[0]} = {inputs[0]};\n".format(
                **self.fmt_dict
            )
        elif self.inputs[0].is_vector():
            return "auto {outputs[0]} = {inputs[0]}[0];\n".format(**self.fmt_dict)
        else:
