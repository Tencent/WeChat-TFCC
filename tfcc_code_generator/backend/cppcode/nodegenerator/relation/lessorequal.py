...
class LessOrEqual(NodeGenerator):
    @classmethod
    def accept(cls, node: ir.node.Node):
        return isinstance(node, ir.node.relation.LessOrEqual)
    ...
    def code(self):
        if (
            ...
            return "auto {outputs[0]} = tfcc::relation::less_equal(static_cast<uint8_t>({inputs[0]}), {inputs[1]});\n".format(
                **self.fmt_dict
            )
        elif (
            ...
            return "auto {outputs[0]} = tfcc::relation::less_equal({inputs[0]}, static_cast<uint8_t>({inputs[1]}));\n".format(
                **self.fmt_dict
            )
        ...
        return "auto {outputs[0]} = tfcc::relation::less_equal({inputs[0]}, {inputs[1]});\n".format(
            **self.fmt_dict
        )
