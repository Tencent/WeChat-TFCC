...
class LayerNormalization(NodeGenerator):
    @classmethod
    def accept(cls, node: ir.node.Node):
        return isinstance(node, ir.node.nn.LayerNormalization)

    @property
    def code(self):
        return "auto {outputs[0]} = tfcc::nn::layer_normalization({inputs[0]}, {inputs[1]}, {inputs[2]}, {epsilon}, {inputs[0]}.shape().size() - 1);\n".format(
