...
class SliceV1(Node):
    def update_attributes(self, axis: int, length: int):
    ...
    def inference(self):
        ...
        if self.length == 1:
            ...
            end = min(self.inputs[0].shape[self.axis], start + self.length)
            assert start < end
            shape[self.axis] = end - start
        else:
            shape[self.axis] = self.create_shape_name("slice_v1")
    ...
    @property
    def attributes(self):
        return {"axis": self.axis, "length": self.length}
