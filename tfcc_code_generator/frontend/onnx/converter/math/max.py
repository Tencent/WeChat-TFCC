...
class Max(Converter):
    def __call__(
        ...
        if input_len > 1:
            ...
            max_input_name = node_proto.input[0]
            max_output_name = graph.context.create_symbol_name(node_proto.output[0])
                ...
                if i + 1 == node_count:
                    graph.append_node(
                        ir.node.math.Max(
                            node_name,
                            graph,
                            [max_input_name, node_proto.input[i + 1]],
                            node_proto.output,
                        )
                    )
                else:
                    ...
                    max_input_name = max_output_name
                    max_output_name = graph.context.create_symbol_name(
                        node_proto.output[0]
                    )
        else:
            graph.append_node(
