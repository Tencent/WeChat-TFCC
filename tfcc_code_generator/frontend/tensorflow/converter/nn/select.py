...
# implement for https://tensorflow.google.cn/api_docs/python/tf/raw_ops/Select?hl=en
class Select(Converter):
    def __call__(self, op: Operation, graph: ir.framework.Graph):
        ...
        graph.append_node(ir.node.nn.Where(op.name, graph, inp_strs, oup_strs))
        return True
