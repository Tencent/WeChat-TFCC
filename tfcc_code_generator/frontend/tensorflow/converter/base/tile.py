...
class Tile(Converter):
    def __call__(self, op: Operation, graph: ir.framework.Graph):
        ...
        shape_symbol = graph.get_symbol(inp_strs[1])
        if shape_symbol.is_vector():
            graph.append_node(ir.node.base.Tile(op.name, graph, inp_strs, oup_strs))
        else:
            to_vector_output = graph.context.create_symbol_name(inp_strs[0])
            graph.append_node(
