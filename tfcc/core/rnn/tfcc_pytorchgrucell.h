...
template <class T>
class PyTorchGRUCell {
  bool _initialized;
  unsigned _units;
  std::string _name;

  Constant<T>* _inputKernel;
  Constant<T>* _stateKernel;
  Constant<T>* _gatesBias;
  Constant<T>* _candidateiBias;
  Constant<T>* _candidatehBias;

 public:
  typedef Variable<T> State;

  explicit PyTorchGRUCell(unsigned units);
  PyTorchGRUCell(unsigned units, const std::string& name);
