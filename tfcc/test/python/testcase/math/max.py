...
class Max(test.Test):
    ...
    def getRandomTestData(self):
        ...
        expect = np.maximum(a, b)
        return {"type": 0, "a": a, "b": b, "expect": expect}
    ...
    def getRandomTestData4(self):
        ...
        expect = np.maximum(a, b)
        return {"type": 1, "a": a, "b": np.asarray([b]), "expect": expect}
    ...
    def getRandomTestData5(self):
        ...
        b = np.random.uniform(-1.0, 1.0, shape)

        expect = np.maximum(a, b)
        return {"type": 2, "a": np.asarray([a]), "b": b, "expect": expect}
