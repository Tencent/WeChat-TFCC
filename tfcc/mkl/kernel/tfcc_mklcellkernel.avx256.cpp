...
#include "tfcc_mklcellkernel.avx256.h"
...
void _MKLCellKernelAVX256<float>::processLSTMCell(
  ...
  _processLSTMCell<MKLVcl8f>(
      batch, units, stateC, inputValue, stateHValue, bias, forget, result, resultState);
}

void _MKLCellKernelAVX256<float>::processGRUCellGates(
  ...
  _processGRUCellGates<MKLVcl8f>(batch, units, inputSize, state, inputs, value, bias, result);
}

void _MKLCellKernelAVX256<float>::processGRUCellCandidate(
  ...
  _processGRUCellCandidate<MKLVcl8f>(batch, units, state, value, bias, cValue, cBias, result);
}

void _MKLCellKernelAVX256<float>::processPyTorchGRUCell(
  ...
  _processPyTorchGRUCell<MKLVcl8f>(
      batch, units, state, inputValue, stateValue, gateBias, candidateIBias, candidateHBias,
      result);
}

// double
void _MKLCellKernelAVX256<double>::processLSTMCell(
    ...
    double* resultState) {
  _processLSTMCell<MKLVcl4d>(
      batch, units, stateC, inputValue, stateHValue, bias, forget, result, resultState);
}

void _MKLCellKernelAVX256<double>::processGRUCellGates(
  ...
  _processGRUCellGates<MKLVcl4d>(batch, units, inputSize, state, inputs, value, bias, result);
}

void _MKLCellKernelAVX256<double>::processGRUCellCandidate(
  ...
  _processGRUCellCandidate<MKLVcl4d>(batch, units, state, value, bias, cValue, cBias, result);
}

void _MKLCellKernelAVX256<double>::processPyTorchGRUCell(
  ...
  _processPyTorchGRUCell<MKLVcl4d>(
      batch, units, state, inputValue, stateValue, gateBias, candidateIBias, candidateHBias,
      result);
}
